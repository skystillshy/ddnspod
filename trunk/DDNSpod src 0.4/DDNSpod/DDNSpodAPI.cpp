...
/*
由于DNSpod提供的API要求的参数是domain_id，即域名在DNSpod中的序号
因此必须首先获得域名对应的domain_id
record_id同样也是记录对应的id，也需要查找返回的记录列表才能得到
*/

CDNSpodAPI::CDNSpodAPI()
{
	ServerURL="www.dnspod.com";
	nPort=80;
	login_email="";
	login_password="";
	bLoginSuccess=FALSE;
...
//返回记录的ID
int CDNSpodAPI::GetRecordID(TiXmlDocument recordlist, CString RecordValue)
{
	if (!bLoginSuccess)
		return 0;
