...
BOOL CDDNSpodDlg::GetIP()
{
	...
	ip4=inet_ntoa(*(in_addr *)name->h_addr_list[0]);
	//MessageBox(ip4);
...
void CDDNSpodDlg::OnFileRefresh()
{
	...
	if (m_conf.A_Record_enabled)
		m_conf.A_RecordID=DNSpod.GetRecordID(RecordList,m_conf.A_Record);
	if (m_conf.AAAA_Record_enabled)
		m_conf.AAAA_RecordID=DNSpod.GetRecordID(RecordList,m_conf.AAAA_Record);
